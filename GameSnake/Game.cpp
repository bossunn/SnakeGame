...
void gameUpdate(GAME* g)
{
	...
	Uint32 color;
	if (g->f.loc.x < 0 || g->f.loc.x >= BOARD_HORZ) // nảy ngược lại nếu chạm các cạnh cửa sổ
	{
		vx = -vx;
		do
		{
			color = rand() % 14 + 1;
		} while (color == g->f.color);
		g->f.color = color;
	}
	if (g->f.loc.y < 0 || g->f.loc.y >= BOARD_VERT) // nảy ngược lại nếu chạm các cạnh cửa sổ
	{
		vy = -vy;
		do
		{
	...
	fruitUpdate(&g->f, vx, vy); /* cập nhật trạng thái di chuyển của mồi */
}
